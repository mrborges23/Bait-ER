...
List sigma_posterior(double N, vec time, int number_time_points, int number_replicates,mat trajectories_matrix, double prior_rate) {
  ...
  vec test_sigma;
  test_sigma << 0 << mean;
  vec test_posterior = coefficients(9) + coefficients(8)*test_sigma + coefficients(7)*pow(test_sigma,2)+coefficients(6)*pow(test_sigma,3)+coefficients(5)*pow(test_sigma,4)+coefficients(4)*pow(test_sigma,5)+coefficients(3)*pow(test_sigma,6)+coefficients(2)*pow(test_sigma,7)+coefficients(1)*pow(test_sigma,8)+coefficients(0)*pow(test_sigma,9);
  double bayes_factor = 2*(test_posterior(1) - test_posterior(0));
  ...
  return List::create(Named("mean")                    = mean,
                      ...
                      Named("q05")                     = q_05,
                      Named("q95")                     = q_95,
                      Named("bayes_factor")            = bayes_factor,
